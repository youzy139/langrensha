...
async def main() -> int:
    async with websockets.connect(f"ws://127.0.0.1:{PORT}/ws") as ws:
        # 第一局：指定当狼
        got, asks = await play_game(ws, "werewolf")
        assert got["role"].get("role") == "werewolf", "指定当狼失败！"
        assert asks > 0, "没有收到任何提问！"
        assert len(got["review"]["md"]) > 50, "复盘内容为空！"
        print(f"第一局: 当狼 ✓ {asks} 次提问 ✓ 复盘 {len(got['review']['md'])} 字 ✓ "
              f"胜者={got['game_over']['winner']}")
        ...
        got2, _ = await play_game(ws, "random")
